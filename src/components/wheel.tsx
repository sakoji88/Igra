...
export function WheelSpinner({
  ...
  const handleSpin = () => startTransition(async () => {
    ...
    const rewardLabel = payload.entry.rewardType === 'ITEM' ? payload.entry.itemName ?? payload.entry.label : payload.entry.rewardType === 'EVENT' ? `событие «${payload.entry.label}»` : payload.entry.label;
            ...
            {entries.map((entry) => (
              <div key={entry.id} className={cn('rounded-2xl border border-zinc-800 bg-zinc-900/70 p-4 transition-colors', selectedEntryId === entry.id && 'border-fuchsia-400/40 bg-fuchsia-500/10')}>
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="rounded-full border border-zinc-700 px-2 py-1 text-[10px] uppercase tracking-[0.24em] text-zinc-300">{entry.rewardType === 'ITEM' ? 'Предмет' : entry.rewardType === 'EVENT' ? 'Событие' : entry.rewardType === 'SPINS' ? 'Спины' : 'Ничего'}</span>
                </div>
